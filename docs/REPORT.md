...
| `PackageQueue`          | The `isEmpty()` check, the read and the `remove(0)` inside `takeNext()`, plus the size read in `pendingCount()` | I1 — a parcel can't be taken by two robots at the same time or disappear from the list without being delivered | `synchronized` on the two methods that touch `pending` | We only synchronized the methods that read or change the list. The constructor doesn't need it since no robots are running yet at that point. If we had only synchronized the `remove` and not the `isEmpty`+`get` together, another robot could still sneak in between those two steps, so the whole sequence has to be atomic |
| `DeliveryRegistry`      | Reading `nextPosition`, incrementing it, and the `deliveries.add()` inside `register()`, plus iterating `deliveries` in `snapshot()` | I3 / I4 — arrival positions must be unique and form the `1..N` sequence with no gaps | `synchronized` on `register()` and `snapshot()` | We protected all of `register()` together because splitting the read of `nextPosition` from the increment and the `add` doesn't make sense — if another robot gets in between those lines it grabs the same position. `snapshot()` is synchronized too, because if a robot is halfway through `register()` when another one calls `snapshot()`, the copy could come out inconsistent. Each class uses its own monitor, separate from `PackageQueue`'s, so robots that just want a new parcel don't get blocked by one that's registering a delivery |
